    ...
    def simulate_action(self, current_state: np.ndarray, 
        ...
        with torch.no_grad():
            for _ in range(num_samples):
                ...
                # Sample from distribution - pred_dist is [B, num_colors, H, W]
                # Reshape to [B, num_colors, H*W] for sampling
                B = pred_dist.shape[0]
                pred_dist_flat = pred_dist.view(B, self.num_colors, -1)
                
                # Sample one color per position
                sampled_flat = torch.multinomial(
                    pred_dist_flat.transpose(1, 2).reshape(-1, self.num_colors),
                    1
                ).view(B, self.grid_size, self.grid_size)
                
                outcomes.append(sampled_flat.cpu().numpy())
